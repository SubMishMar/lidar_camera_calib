clear all;
clc;
files1 = dir('../left/lines/lidar/line1_*.csv');
files2 = dir('../left/lines/lidar/line2_*.csv');
files3 = dir('../left/lines/lidar/line3_*.csv');
files4 = dir('../left/lines/lidar/line4_*.csv');
...
%  [A1, B1] = fit3DLine(csv1);
  ...
  csv2 = load((strcat('../left/lines/lidar/', files2(i).name)));
  xP2 = csv2(:, 1); yP2 = csv2(:, 2); zP2 = csv2(:, 3);
%  [A2, B2] = fit3DLine(csv2);
  
  csv3 = load((strcat('../left/lines/lidar/', files3(i).name)));
  xP3 = csv3(:, 1); yP3 = csv3(:, 2); zP3 = csv3(:, 3);
%  [A3, B3] = fit3DLine(csv3);
  
  csv4 = load((strcat('../left/lines/lidar/', files4(i).name)));
